...
@Slf4j
public class MedicineInsuranceCoverageSyncInService {
    ...
    @Transactional
    public void syncMedicineInsuranceCoverages(String clientId) {
        try {
            log.info("Starting medicine insurance coverage sync for client: {}", clientId);

            // Get baseUrl from database using client ID
            String baseUrl = getBaseUrlFromClient(clientId);

            // Fetch data from source API
            MedicineInsuranceCoveragePageResponse response = fetchMedicineInsuranceCoveragesFromSource(baseUrl);

            if (response != null && response.getContent() != null) {
                log.info("Fetched {} medicine insurance coverages from source for client: {}", response.getContent().size(), clientId);
                ...
                log.info("Medicine insurance coverage sync completed successfully for client: {}", clientId);
            } else {
                log.warn("No medicine insurance coverage data received from source for client: {}", clientId);
            }

        } catch (Exception e) {
            log.error("Error during medicine insurance coverage sync for client: {}", clientId, e);
            throw new RuntimeException("Medicine insurance coverage sync failed for client: " + clientId, e);
        }
    }
    ...
    private MedicineInsuranceCoveragePageResponse fetchMedicineInsuranceCoveragesFromSource(String baseUrl) {
        try {
            ...
            String url = String.format("%s/inventory/api/sync/medicine-insurance-coverages?page=0&size=1000", formattedBaseUrl);
